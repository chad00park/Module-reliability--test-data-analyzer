...
def parse_filename(path):
    ...
    if is_retest:
        rel = "RETEST_" + rel
    return rel, lot, readout_label, readout_value
...
class DataModel:
    ...
    def load(self, files):
        ...
        for p in files:
            ...
            raw.setdefault((rel, lot), []).append((rl, rv, phases, cols, d, fname))

        if not raw:
            return errors
        # Retest_X와 X는 같은 신뢰성으로 취급(별도 분석 단위), base가 다르면 차단
        bases = {}
        ...
        for rel_key, lot in sorted(raw):
            gkey = lot if not rel_key.upper().startswith("RETEST_") else f"{lot}(R)"
    ...
    def set_limits(self, lot, col, high, low):
        key = (lot, col)
...
def draw_line(ax, model, lot, col, picker=False, screen=False):
    ...
    for r in model.readouts(lot):
        ...
        if out_x:  # limit 밖: 동일 크기의 굵은 * (원래 색 유지)
    ...
    ax.legend(fontsize=9 if screen else 6, ncol=max(1, len(model.readouts(lot))),
              ...
              columnspacing=0.8)
    text_artists = draw_overlays(ax, model, lot, col, "line")
...
def draw_delta(ax, model, lot, col, screen=False, picker=False):
    ...
    text_artists = draw_overlays(ax, model, lot, col, "delta")
    ax.grid(True, alpha=0.3)
    artists["__texts__"] = text_artists
    return artists
...
def draw_box(ax, model, lot, col, phase=None, stacked=False,
    ...
    if (lot, col) in model.ylim:
        ax.set_ylim(*model.ylim[(lot, col)])
    text_artists = draw_overlays(ax, model, lot, col, "box")
    ax.grid(True, alpha=0.3)
        ...
        def apply():
            ...
            if hi is not None and lo is not None and hi <= lo:
                messagebox.showwarning("알림", "High limit는 Low limit보다 커야 합니다.",
                                       parent=dlg)
                return
            self.model.set_limits(lot, col, hi, lo)
            dlg.destroy()
            self._redraw()
